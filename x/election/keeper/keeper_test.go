...
type testFixture struct {
	ctx         sdk.Context
	k           keeper.Keeper
	msgServer   election.MsgServer
	queryServer election.QueryServer
...
func initFixture(t *testing.T) *testFixture {
	...
	key := storetypes.NewKVStoreKey(election.ModuleName)
	...
	err := k.InitGenesis(testCtx.Ctx, election.NewGenesisState())
	if err != nil {
		panic(err)
	}
